...
#setwd("D:\\PROIECTE\\HORIZON 2020\\Use Case DP\\Cluster_Cryptos")
...
Check <-c("BTC","ETH","XRP","USDT","KMD","EOS","BCH") 



index_show=match( Check,symb_assets)
png("Clusters cryptos.png")
s3d<-scatterplot3d(clusters[,4:6], pch = 16, color=as.numeric(clusters$cluster_new),  
...
svm1 <- svm(as.factor(cluster_new)~ F1+F2+F3, data=clusters,
            method="C-classification", kernal="radial", 
            gamma=0.1, cost=10)
prediction <- predict(svm1, clusters[,4:6])

xtab <- table(clusters$cluster_new, prediction)
xtab
grid.table(xtab)
#The validity of this classification is proven by Support Vector Machines (SVM)
Accuracy=sum(diag(xtab))/length(symb_assets)
print(Accuracy)
